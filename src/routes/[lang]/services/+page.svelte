<script lang="ts">
	...
	const serviceCards: ServiceCard[] = [
		...
		{
			key: 'digitization',
			icon: `<svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M13 3H7a2 2 0 00-2 2v14a2 2 0 002 2h10a2 2 0 002-2V8z" /><path stroke-linecap="round" stroke-linejoin="round" d="M13 3v5h5M9 13h6M9 17h6" /></svg>`
		}
	];
</script>
		...
		<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
			{#each serviceCards as card, i}
				...
				<div class="card p-8 {i % 2 === 1 ? 'bg-surface-alt border-surface-alt' : ''}">
					...
					<p class="text-gray-mid leading-relaxed mb-5">{service.pain as string}</p>
					<p class="text-gray-mid leading-relaxed font-medium">{service.solution as string}</p>
				</div>
			{/each}

			<div class="text-center pt-4">
				<a href="/{data.lang}/contact" class="btn-primary">
					{s.cta as string}
				</a>
			</div>
		</div>
